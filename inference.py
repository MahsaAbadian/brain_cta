"""
Script for running inference for the TopBrain multiclass segmentation.
...
The relevant paths are as follows:
    input_path: contains the input images inside the folder
    /images/head-mr-angio or /images/head-ct-angio (depending on your TRACK)
        Docker: /input
        Local: ./test/input
    output_path: output predictions are stored inside the folder
    /images/head-mr-angio-segmentation or /images/head-ct-angio-segmentation (depending on your TRACK)
        Docker: /output
        Local: ./test/output
...
from your_algorithm import TRACK, do_you_use_pytorch_cuda, your_segmentation_algorithm

track = TRACK.lower()
assert track in ("mr", "ct"), f"Unknown track: {TRACK}! Must be 'MR' or 'CT'."

print(f"user algo track = {track}")
...
def run():
    ...
    # Read the input
    # Depending on the value of TRACK it loads the MR or CT angiography
    # Gives a npy array with shape (x,y,z)
    input_head_angiography = load_image_file_as_array(
        location=input_path / f"images/head-{track}-angio",
    )
    ...
    pred_array = your_segmentation_algorithm(input_array=input_head_angiography)
...
def write_array_as_image_file(*, array, input_folder, output_folder):
    ...
    input_img = load_image_file(input_path=input_folder / f"images/head-{track}-angio")
    required_output_shape = input_img.GetSize()
    print(f"Check {array.shape} == {required_output_shape}...")
    assert (
        array.shape == required_output_shape
    ), "Prediction output must have the same shape as the input image!"

    # Create the output folder
    output_location = output_folder / f"images/head-{track}-angio-segmentation"
